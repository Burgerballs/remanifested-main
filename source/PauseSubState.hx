...
class PauseSubState extends MusicBeatSubstate
{
	...
	var menuBlor:Array<String> = 
	[
		'Continues with the song', 
		'Plays the song back from the beginning',
		'Changes the difficulty of the song, then restarts the song',
		'Stops the song and enters the Freeplay/Story menu'
	];
	...
	public function new(x:Float, y:Float)
	{
		...
		add(infoText);
		infoText.alpha = 0;
		FlxTween.tween(infoText, {alpha: 1}, 0.4, {ease: FlxEase.quartInOut});
		...
		if (accepted && (cantUnpause <= 0 || !ClientPrefs.controllerMode))
		{
			...
			switch (daSelected)
			{
				...
				case 'Toggle Botplay':
					...
					PlayState.changedDifficulty = true;
					PlayState.instance.suckerTxt.visible = PlayState.instance.cpuControlled;
					PlayState.instance.suckerTxt.alpha = 1;
	...
	function changeSelection(change:Int = 0):Void
	{
		...
		if (curSelected >= menuItems.length)
			curSelected = 0;
		infoText.text = menuBlor[curSelected];
