...
import meta.playerdata.Highscore;
...
import meta.editors.ChartingState;
import meta.editors.CharacterEditorState;
...
import gameobj.DialogueBoxPsych.DialogueFile;
...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['AAA', 0.85], // From 70% to 79%
		['AAAA', 0.9], // From 70% to 79%
		['S', 0.93], // From 80% to 89%
		['SS', 0.95], // From 80% to 89%
		['SSS', 0.98], // From 80% to 89%
		['SSSS', 1], // From 90% to 99%
	];
	...
	public var un4givin:Bool = false;
	public var cpuControlled:Bool = false;
	public var practiceMode:Bool = false;

	public var botplaySine:Float = 0;
	public var suckerTxt:FlxText;
	...
	override public function create()
	{
		...
		un4givin = ClientPrefs.getGameplaySetting('un4giving', false);
		...
		switch (curStage)
		{
			...
			case 'mario':
				var atmosphere = new FlxSprite(0, -500).loadGraphic(Paths.image('bg/sky'));
				atmosphere.setGraphicSize(Std.int(2 * atmosphere.width));
				add(atmosphere);

				var blocks = new FlxBackdrop(Paths.image('bg/haxeground'), 100, 0, true, false);
				blocks.setGraphicSize(Std.int(10 * blocks.width));
				blocks.y = 560;
				blocks.velocity.x = -1280;
				add(blocks);
				blocks.antialiasing = false;
				var bush = new FlxBackdrop(Paths.image('bg/bush'), 100, 0, true, false, 1600);
				bush.setGraphicSize(Std.int(10 * bush.width));
				bush.y = 400;
				bush.velocity.x = -1280;
				add(bush);
				bush.antialiasing = false;

				var cloud = new FlxBackdrop(Paths.image('bg/cloud'), 100, 0, true, false, 1300);
				cloud.setGraphicSize(Std.int(10 * cloud.width));
				cloud.y = 100;
				cloud.velocity.x = -1280;
				add(cloud);
				cloud.antialiasing = false;

				var balls = new FlxBackdrop(Paths.image('bg/blocks'), 100, 0, true, false, 1700);
				balls.setGraphicSize(Std.int(10 * balls.width));
				balls.y = 50;
				balls.velocity.x = -1280;
				add(balls);
				balls.antialiasing = false;

				atmosphere.scrollFactor.set(0, 0);
				blocks.scrollFactor.set(0, 0);
				bush.scrollFactor.set(0, 0);
				cloud.scrollFactor.set(0, 0);
				balls.scrollFactor.set(0, 0);
		}
		...
		if (Paths.formatToSongPath(SONG.song) == 'Manifest-high') {
			var evilTrail = new FlxTrail(dad, null, 8, 24, 0.3, 0.069); // nice
			addBehindDad(evilTrail);
		}
		...
		suckerTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "SUCKER", 32);
		suckerTxt.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		suckerTxt.scrollFactor.set();
		suckerTxt.borderSize = 1.25;
		suckerTxt.visible = cpuControlled;
		add(suckerTxt);
		if (ClientPrefs.downScroll)
		{
			suckerTxt.y = timeBarBG.y - 78;
		}
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				if (!Std.isOfType(songNotes[3], String))
					swagNote.noteType = meta.editors.ChartingState.noteTypeList[songNotes[3]]; // Backward compatibility + compatibility with Week 7 charts
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			startTimer.active = !(startTimer != null && !startTimer.finished) || !(finishTimer != null && !finishTimer.finished) || !(songSpeedTween != null); 
	...
	override public function update(elapsed:Float)
	{
		...
		if (suckerTxt.visible)
		{
			botplaySine += 180 * elapsed;
			suckerTxt.alpha = 1 - Math.sin((Math.PI * botplaySine) / 180);
		}
		...
		if (inCutscene == false && generatedMusic == true)
		{
			keyShitter();
		}
		checkEventNote();
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String)
	{
		switch (eventName)
		{
			case 'Dadbattle Spotlight':
				...
				switch (Std.parseInt(value1))
				{
					case 1, 2, 3: // enable and target dad
						...
						dadbattleBlack.visible = (val == 1);
						dadbattleLight.visible = (val == 1);
						dadbattleSmokes.visible = (val == 1);
						defaultCamZoom += 0.12;
			...
			case 'Set Property':
				...
				if (killMe.length > 1)
				{
					FunkinLua.setVarInArray(this, FunkinLua.getPropertyLoopThingWhatever(killMe), value2);
				}
				else
				{
	...
	public function popUpScore(note:Note = null):Void
	{
		...
		if (daRating.name == 'shit' && un4givin || daRating.name == 'bad' && un4givin)
			{
				noteMiss(note);
			...
			&& (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || ClientPrefs.controllerMode))
		{
			if (!boyfriend.stunned && generatedMusic && !endingSong)
			{
				...
				notes.forEachAlive(function(daNote:Note)
				{
					...
					canMiss = (strumsBlocked[daNote.noteData] != true
						&& daNote.canBeHit
						&& daNote.mustPress
						&& !daNote.tooLate
						&& !daNote.wasGoodHit
	...
	private function keyShit():Void
	{
		...
		if (startedCountdown && !boyfriend.stunned && generatedMusic)
		{
			...
			if (parsedHoldArray.contains(true) && !endingSong)
			{
				...
				&& boyfriend.holdTimer > Conductor.stepCrochet * (0.0011 / playbackRate) * boyfriend.singDuration
				&& boyfriend.animation.curAnim.name.startsWith('sing')
				&& !boyfriend.animation.curAnim.name.endsWith('miss'))
			{
				boyfriend.dance();
	...
	{
		camZooming = (Paths.formatToSongPath(SONG.song) != 'tutorial');
	...
	public var ratingPercent:Float = 0.00;
	...
	public function RecalculateRating(badHit:Bool = false)
	{
		...
		if (ret != FunkinLua.Function_Stop)
		{
			...
			comboBreaks = bads + shits + songMisses;
			...
			if (bads > 0 || shits > 0)
				ratingFC = "FC";
			if (comboBreaks > 0 && comboBreaks < 10)
				ratingFC = "SDCB";
