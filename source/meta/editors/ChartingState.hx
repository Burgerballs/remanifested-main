...
import gameobj.Character.*;
import gameobj.Character;
#if desktop
import Discord.DiscordClient;
#end
import meta.editors.chartingobj.ChartingNote;
...
class ChartingState extends MusicBeatState
{
	...
	var gradients:FlxTypedGroup<FlxSprite>;
	...
	**/
	...
	var coolGradient2:FlxSprite;
	var coolGradient3:FlxSprite;
	...
	override function create()
	{
		...
		coolGradient = FlxGradient.createGradientFlxSprite(GRID_SIZE, 720,
		FlxColor.gradient(0xC20CC6FA, FlxColor.TRANSPARENT, 32));
		coolGradient.scrollFactor.set();
		add(coolGradient);
		coolGradient.setPosition(GRID_SIZE * 7, 0);

		gradients = new FlxTypedGroup<FlxSprite>();

		coolGradient2 = FlxGradient.createGradientFlxSprite(GRID_SIZE * 4, 720,
		FlxColor.gradient(0xFF00FF00, FlxColor.TRANSPARENT, 32));
		coolGradient2.scrollFactor.set();
		gradients.add(coolGradient2);
		coolGradient2.setPosition(GRID_SIZE * 8, 0);

		coolGradient3 = FlxGradient.createGradientFlxSprite(GRID_SIZE * 4, 720,
		FlxColor.gradient(0xFFFF0000, FlxColor.TRANSPARENT, 32));
		coolGradient3.scrollFactor.set();
		gradients.add(coolGradient3);
		coolGradient3.setPosition(GRID_SIZE * 12, 0);
	...
	function updateTxt() {
		var diff = meta.states.calcers.DiffCalc.CalculateDiff(_song);
	...
	{
		gridLayer.clear();
		var gridBGBehind = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 9, Std.int(GRID_SIZE * getSectionBeats() * 8 * zoomList[curZoom]));
		gridBGBehind.y -= GRID_SIZE * getSectionBeats() * 8 * zoomList[curZoom];
		...
		if (sectionStartTime(1) <= FlxG.sound.music.length)
		{
			...
			leHeight = Std.int(gridBGBehind.height + nextGridBG.height);
		}
		else
			nextGridBG = new FlxSprite().makeGraphic(1, 1, FlxColor.TRANSPARENT);
		nextGridBG.y = gridBG.height;
		gridLayer.add(gridBGBehind);
		gridLayer.add(nextGridBG);
		gridLayer.add(gridBG);

		gridBGBehind.alpha = 0.3;
		gridBG.alpha = 0.3;
		nextGridBG.alpha = 0.3;


		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + (GRID_SIZE * 5), 0 - GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]).makeGraphic(2, Std.int(leHeight*1.5), FlxColor.BLACK);
		...
		for (i in 1...16)
		{
			...
			{
				if (i % 4 != 0) {
					var stepsep:FlxSprite = new FlxSprite(gridBG.x, GRID_SIZE * i).makeGraphic(Std.int(gridBG.width), 2, 0xBCFF2828);
					gridLayer.add(stepsep);
				}
				var beatsep1:FlxSprite = new FlxSprite(gridBG.x - GRID_SIZE, (GRID_SIZE * (2 * curZoom)) * i).makeGraphic(Std.int(gridBG.width + (gridBG.width / 9)), 3, 0x8E1E00FF);
				gridLayer.add(beatsep1);
			}
		}

		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + GRID_SIZE, 0 - GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]).makeGraphic(2, Std.int(leHeight*1.5), FlxColor.BLACK);
		gridLayer.add(gridBlackLine);
		updateGrid();
		var edge1:FlxSprite = new FlxSprite(gridBG.x, gridBG.y).makeGraphic(Std.int(gridBG.width), 4, 0xFF00FF40);
		var edge2:FlxSprite = new FlxSprite(gridBG.x, gridBG.height).makeGraphic(Std.int(gridBG.width), 4, 0xFF00FF40);
		gridLayer.add(edge1);
		gridLayer.add(edge2);
	...
	function updateGradients() {
		coolGradient2.destroy();
		coolGradient3.destroy();
		if (_song.notes[curSec].mustHitSection)
			{
				coolGradient2 = FlxGradient.createGradientFlxSprite(GRID_SIZE * 4, 720,
					FlxColor.gradient(0xFF00FF00, FlxColor.TRANSPARENT, 32));
	...
	function loadHealthIconFromCharacter(char:String) {
		...
		var json:gameobj.Character.CharacterFile = cast Json.parse(rawJson);
	...
	function updateGrid():Void
	{
		...
		if (curSec > 0 && curSec < _song.notes.length - 1)
		{
