...
function onEvent(name, value1, value2)
	if name == "Toggle Trail-moving" then
...
function createTrailFrame(tag)
	...
	if num - trailLength + 1 >= 0 then
		for i = (num - trailLength + 1), (num - 1) do
			setProperty('psychicTrail'..tag..i..'.alpha', getProperty('psychicTrail'..tag..i..'.alpha') - (0.8 / (trailLength - 1)));
		end
	end
	...
	if not (image == '') then
		...
		setBlendMode(trailTag, 'subtract');
		doTweenX(trailTag..'bingo', trailTag, getProperty(trailTag..'.x') - 1280, 0.75, 'sineInOut')
		doTweenY(trailTag..'bingoo', trailTag, getProperty(trailTag..'.y') - 200, 5, 'sineOut')
